...
cull _ Empty = Empty
cull val (Node x l r) | x == val = Empty
...
set _ _ Empty = Empty
set [] x (Node _ l r) = Node x l r
set (StepL:xs) x (Node y l r) = Node y (set xs x l) r
set (StepR:xs) x (Node y l r) = Node y l (set xs x r)
