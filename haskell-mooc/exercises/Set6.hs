...
instance Num RationalNumber where
  (RationalNumber x y) + (RationalNumber x' y') = simplify (RationalNumber (x * y' + x' * y) (y * y'))
  (RationalNumber x y) * (RationalNumber x' y') = simplify (RationalNumber (x * x') (y * y'))
...
class Cycle a where
  step :: a -> a
  stepMany :: Int -> a -> a
  stepMany n x = foldl (\x _ -> step x) x [1..n]
