...
mapMaybe f x | isNothing x = Nothing
             | otherwise = Just (f (fromJust x))
...
mapMaybe2 f x y | isNothing x = Nothing
                | isJust x && isNothing y = Nothing
...
firstHalf s | even (length s) = take (length s `div` 2) s
...
multiApp f gs x = f $ reverseMap gs x
                      where
                        reverseMap [] _ = []
                        reverseMap (g:gs) x = g x : reverseMap gs x
...
interpreter commands = interpreterHelper commands 0 0
                       where
                            ...
                            interpreterHelper ("up":xs) x y = interpreterHelper xs x (y+1)
                            interpreterHelper ("down":xs) x y = interpreterHelper xs x (y-1)
                            interpreterHelper ("left":xs) x y = interpreterHelper xs (x-1) y
                            interpreterHelper ("right":xs) x y = interpreterHelper xs (x+1) y
                            interpreterHelper ("printX":xs) x y = show x : interpreterHelper xs x y
                            interpreterHelper ("printY":xs) x y = show y : interpreterHelper xs x y
