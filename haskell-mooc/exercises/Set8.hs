...
combine f (Picture f1) (Picture f2) = Picture f3
  where f3 (Coord x y) = f (f1 (Coord x y)) (f2 (Coord x y))
...
paintSolid color shape (Picture f) = Picture g
  where g (Coord x y) | contains shape x y = color
                      | otherwise = f (Coord x y)
------------------------------------------------------------------------------
...
instance Transform Fill where
  apply (Fill color) = \pic -> Picture (\coord -> color)
...
instance Transform Blur where
  apply Blur = \(Picture pic) -> Picture (\(Coord x y) -> average (pic (Coord x y)) (pic (Coord (x-1) y)) (pic (Coord (x+1) y)) (pic (Coord x (y-1))) (pic (Coord x (y+1))))
    where
...
instance Transform BlurMany where
  apply (BlurMany n) = \pic -> foldl (\acc x -> apply Blur acc) pic (replicate n Blur)
------------------------------------------------------------------------------
